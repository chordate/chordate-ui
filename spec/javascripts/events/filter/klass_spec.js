describe("EventsFilterKlassView", function () {
  var view, request;
  ...
  beforeEach(function () {
    ...
    view = new c.v.EventsFilterKlassView();
  ...
  it("should have the title", function () {
    expect(view.title).toEqual("Error Class");
  });
  ...
  it("should have the application events target", function () {
    expect(view.target()).toEqual("/applications/123/filters.json?model=event&type=klass");
  });
  ...
  it("should get the target", function () {
    ...
    expect(request.url).toMatch(/^\/applications\/123\/filters.json\?model=event&type=klass/);
  });
  ...
  describe("#render", function () {
    ...
    describe("when the filters are returned", function () {
      ...
      it("should have the filter names", function () {
        expect(error(0)).toHaveText("ErrorClass 44");
        expect(error(1)).toHaveText("ErrorClass 21");
      });

      it("should have the names in data", function () {
        expect(error(0).data("klass")).toEqual("ErrorClass 44");
        expect(error(1).data("klass")).toEqual("ErrorClass 21");
      });
  ...
  describe("#filter", function () {
    ...
    it("should set the filter on the collection", function () {
      expect(events.filters).toEqual({klass: "ErrorClass 44"})
    });
